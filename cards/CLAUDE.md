...
3. Timelines must be paused and registered on `window.__timelines`:
   ...
   ```
4. Videos: if the `<video>` should carry its own audio through to the render, drop the `muted` attribute and add `data-has-audio="true"`. Lint will reject a non-muted `<video>` without that attribute. Use a separate `<audio>` element only when you need an audio track that's independent of the video (e.g., voiceover over a muted clip, or multiple stacked videos with chosen audio).
