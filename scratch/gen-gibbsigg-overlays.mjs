import fs from "fs";
// 2025 Topps Signature Class (GibbSigg) overlay generator — adapts scratch/gen-signature2025-overlays.mjs.
// Sits on the host's already-cut video (cards/sources/gibbsigg/Gibbsiggshort.mp4, native portrait
// 2160x3840 9:16 so object-fit:cover fits exactly; keeps its audio via data-has-audio). comp chip
// (eBay screenshot + value) top-right per card (inset right:240@4K per UI safe-zone), running
// box-cost tracker top-left, profit/ROI recap on black after the last card.
//
// FAST CUTS: this cut is quicker than Signature2025 — chips fire almost on the card landing
// (CHIP_DELAY 0.06), snappier in/out (IN 0.22 / OUT 0.30), shorter floor (0.7s) so back-to-back
// comps (Mahomes 47.5 -> Breece 48.5, ~1.0s apart) don't collide.
//
// SCALE (argv[2] or env SCALE, default 1): 1 -> 1080x1920. 2 -> author NATIVELY at 2160x3840 true 4K.
// Prefer argv so the command stays atomic (no `SCALE=2 node ...` env-prefix, which defeats Bash(node:*)
// allow-rules and triggers a permission prompt): run `node scratch/gen-gibbsigg-overlays.mjs 2`.
// COST GATE: tracker + recap only render when comps.boxCost > 0. Until the user confirms the box
// price, run as-is for a chips-only preview; set boxCost then re-run to switch the tracker/recap on.
const S = +(process.argv[2] || process.env.SCALE || 1);
const z = n => n * S;
...
const COST = comps.boxCost;            // box price (0 until user confirms)
const HASCOST = COST > 0;
const VIDEO = 57.45;                   // host's cut duration
const PREVIEW = process.env.PREVIEW === "1";
const BARE = process.env.BARE === "1"; // tracker-only render (omit comp chips)
const RECAP_START = VIDEO;             // recap begins on black right after the last card
const DURATION = process.env.DUR ? +process.env.DUR
  : (PREVIEW ? 22.0 : (HASCOST ? VIDEO + 6.0 : VIDEO)); // +6s black recap tail only when cost known
const TRACK_FADE = RECAP_START - 0.4;
const TRACK_START = 5.0;                // box-cost tracker enters as the first card shows (~5s)
const fmt = v => "$" + v.toFixed(2);
const W = z(1080), H = z(1920);

// fast-cut chip timing
const CHIP_DELAY = 0.06, DUR_MIN = 0.7, DUR_MAX = 3.0, CHIP_GAP = 0.04;
...
const chipStartOf = c => (c.chipStartOutput != null ? c.chipStartOutput : c._s + CHIP_DELAY);
for (let i = 0; i < cards.length; i++) {
  const c = cards[i]; const next = cards[i + 1];
  c.start = +chipStartOf(c).toFixed(2);
  let dur = c._e - c.start;
  dur = Math.min(Math.max(dur, DUR_MIN), DUR_MAX);
  if (next) dur = Math.min(dur, chipStartOf(next) - c.start - CHIP_GAP);
  c.dur = +Math.max(dur, 0.55).toFixed(2);
}
...
const saleDivs = BARE ? "" : cards.map((c, i) => `      <div id="chip-${c.cardId}" class="sale-card clip" data-start="${c.start}" data-duration="${c.dur}" data-track-index="${i + 1}" data-value="${c.value}"${c.money ? ' data-money-shot="true"' : ''}>
        <div class="sale-card__comp"><img src="../comps/GibbSigg/${c.file}" alt="${c.player} ${c.parallel} ${fmt(c.value)}" crossorigin="anonymous" /></div>
${c.label ? `        <div class="sale-card__label">${c.label}</div>\n` : ''}        <div class="sale-card__value">${fmt(c.value)}</div>
      </div>`).join("\n");

const trackerDiv = HASCOST ? `
...
<html lang="en" data-resolution="portrait">
  <head>
    <meta charset="UTF-8" />
    <title>Final Edit (Shorts) - GibbSigg${S > 1 ? " (4K)" : ""}</title>
  ...
  <body>
    <div id="root" data-composition-id="final-GibbSigg" data-width="${W}" data-height="${H}" data-start="0" data-duration="${DURATION}">
      <video id="source-video" class="clip" data-start="0" data-duration="${VIDEO}" data-track-index="0" data-has-audio="true" playsinline src="../sources/gibbsigg/Gibbsiggshort.mp4"></video>
...
${recapDiv}
      ...
      <script>
        (function () {
          const SCL=${S};
          const HASCOST=${HASCOST};
          const IN_DUR=0.22, OUT_DUR=0.30, POP_IN=0.6;
          ...
          document.querySelectorAll(".sale-card.clip").forEach((el)=>{
            ...
            const hold=Math.max(0.12,dur-inD-OUT_DUR);
          ...
          window.__timelines["final-GibbSigg"]=tl;
        })();
      </script>
    </div>
  </body>
...
`;
const OUT_HTML = PREVIEW ? "cards/compositions/final-GibbSigg-preview.html"
  : BARE ? `cards/compositions/final-GibbSigg${S > 1 ? "-4k" : ""}-bare.html`
  : (S > 1 ? "cards/compositions/final-GibbSigg-4k.html" : "cards/compositions/final-GibbSigg.html");
fs.writeFileSync(OUT_HTML, html);
const tot = cards.reduce((s, c) => s + c.value, 0);
console.log(`wrote ${OUT_HTML} @ ${W}x${H} (S=${S}) : ${cards.length} comp chips, cost ${HASCOST ? fmt(COST) : "(TBD - chips only)"}, total ${fmt(tot)}${HASCOST ? `, profit ${fmt(tot - COST)} (${((tot - COST) / COST * 100).toFixed(1)}% ROI)` : ""}, money=${cards.find(c => c.money)?.player}`);
