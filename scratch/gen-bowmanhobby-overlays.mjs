import fs from "fs";
// Bowman Chrome U (Hobby SuperCut) overlay generator — adapts scratch/gen-signature2025-overlays.mjs.
// Sits on the host's re-cut BowmanHobbySuperCut.mp4 (native portrait 1080x1920 60fps, 134.19s,
// SILENT track -> source stays muted). Same 23 comped cards as the old bowmanchrome2025 build,
// re-timed to the new cut (scratch/bowmanhobby-card-positions.json). comp chip (eBay screenshot +
// value) top-right per card, running box-cost tracker top-left, profit/ROI recap on black after last card.
//
// SAFE-ZONES (TikTok / Reels / YouTube Shorts): tracker top-left at top:332 (~17% down, clears top
// bars + back button); comp chips inset right:120 (clears right action rails) and top:150 (nudged
// below the YouTube/Instagram top-icon strip). See ui-safezone memory.
//
// SCALE (argv[2], default 1): 1 -> author at 1080x1920 (source downscaled 1:1). 2 -> native 2160x3840
// true 4K (source BowmanHobbyRevamp.mp4 is 4K, used 1:1). Pass as argv: `node gen-…mjs 2` (never env).
...
const COST = comps.boxCost;            // 209.99
const VIDEO = 147.27;                  // BowmanHobbyRevampFinalCut-clean.mp4 (Barnes 1s + 5 black gaps removed)
const PREVIEW = process.env.PREVIEW === "1";
const BARE = process.env.BARE === "1";  // tracker-only render (omit comp chips) — position/UX check
const RECAP_START = VIDEO;             // recap begins on black right after the last card (Vasko)
const DURATION = process.env.DUR ? +process.env.DUR : (PREVIEW ? 22.0 : VIDEO + 6.7); // recap fills VIDEO..+6.7s on black
const TRACK_FADE = RECAP_START - 0.4;
const TRACK_START = 6.0;                // box-cost tracker enters as the first card shows (~6s)
...
const saleDivs = BARE ? "" : cards.map((c, i) => `      <div id="chip-${c.cardId}" class="sale-card clip" data-start="${c.start}" data-duration="${c.dur}" data-track-index="${i + 1}" data-value="${c.value}"${c.money ? ' data-money-shot="true"' : ''}>
        <div class="sale-card__comp"><img src="../comps/bowmanchrome2025/${c.file}" alt="${c.player} ${c.parallel} ${fmt(c.value)}" crossorigin="anonymous" /></div>
...
<html lang="en" data-resolution="portrait">
  <head>
    <meta charset="UTF-8" />
    <title>Final Edit (Shorts) - Bowman Chrome U Hobby${S > 1 ? " (4K)" : ""}</title>
    ...
    <style>
      ...
      .sale-card { position:absolute; right:${z(120)}px; top:${z(150)}px; width:${z(230)}px; padding:${z(9)}px ${z(9)}px ${z(4)}px; border-radius:${z(20)}px;
  ...
  <body>
    <div id="root" data-composition-id="final-bowmanhobby" data-width="${W}" data-height="${H}" data-start="0" data-duration="${DURATION}">
      <video id="source-video" class="clip" data-start="0" data-duration="${VIDEO}" data-track-index="0" muted playsinline src="../sources/bowmanchrome2025/BowmanHobbyRevampFinalCut-clean.mp4"></video>
...
${saleDivs}
      ...
      <script>
        (function () {
          ...
          window.__timelines["final-bowmanhobby"]=tl;
        })();
      </script>
    </div>
  </body>
...
`;
const OUT_HTML = PREVIEW ? "cards/compositions/final-bowmanhobby-preview.html"
  : BARE ? `cards/compositions/final-bowmanhobby${S > 1 ? "-4k" : ""}-bare.html`
  : (S > 1 ? "cards/compositions/final-bowmanhobby-4k.html" : "cards/compositions/final-bowmanhobby.html");
