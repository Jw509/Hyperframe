import fs from "fs";
// Bowman Chrome U overlay generator — adapts scratch/chrome-short/gen_final_html.mjs
// Sits on the under-3 speed cut. comp chip (screenshot+value) top-right per card, running box total top-left, ROI recap at end.
...
const VIDEO = 173.283;                 // v18-under3 duration (Royer shaved 1s more)
const PREVIEW = process.env.PREVIEW === "1";
const DURATION = PREVIEW ? 26.0 : 178.9; // composition; recap fills VIDEO_END..178.9 on black; under 3:00
const RECAP_START = 173.28;             // = video end, so the final card (Jaden Nixon) is fully visible before the black recap
const TRACK_FADE = RECAP_START - 0.4;
const TRACK_START = 10.0;   // price overlay (box-cost tracker) enters at 10s, synced with the cost sting SFX
const fmt = v => "$" + v.toFixed(2);

// build sale cards from comps + their card output positions, sorted by time
...
for (let i=0;i<cards.length;i++){
  ...
  c.start = +chipStartOf(c).toFixed(2);          // honor per-comp override (e.g. Eric, so the chip lands on Eric not the Jayden lead-in)
...
// chip = the eBay screenshot itself (already shows card + "Sold" date + price); value carried as data-value for the tracker/recap math
...
<html lang="en" data-resolution="portrait">
  <head>
    <meta charset="UTF-8" />
    <title>Final Edit (Shorts) - bowman chrome U</title>
  ...
  <body>
    <div id="root" data-composition-id="final-bowmanchrome2025" data-width="1080" data-height="1920" data-start="0" data-duration="${DURATION}">
      <video id="source-video" class="clip" data-start="0" data-duration="${VIDEO}" data-track-index="0" muted playsinline src="../sources/bowmanchrome2025/bowmanchrome2025-catalog-short-v18-under3.mp4"></video>
...
${saleDivs}
      ...
      <script>
        (function () {
          ...
          window.__timelines["final-bowmanchrome2025"]=tl;
        })();
      </script>
    </div>
  </body>
...
`;
const OUT_HTML = PREVIEW ? "cards/compositions/final-bowmanchrome2025-preview.html" : "cards/compositions/final-bowmanchrome2025.html";
fs.writeFileSync(OUT_HTML, html);
const tot = cards.reduce((s,c)=>s+c.value,0);
console.log(`wrote final-bowmanchrome2025.html : ${cards.length} comp chips, cost ${fmt(COST)}, total ${fmt(tot)}, profit ${fmt(tot-COST)} (${((tot-COST)/COST*100).toFixed(1)}% ROI), money=${cards.find(c=>c.money)?.player}`);
