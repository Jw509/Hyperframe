import fs from "fs";
// 2025 Topps Signature Class overlay generator — adapts scratch/gen-2024bowmanu-overlays.mjs.
// Sits on the host's already-cut video (cards/sources/shatteredJtaylor/ShatteredJTaylor.mp4,
// native portrait 2160x3840 9:16 so object-fit:cover fits exactly; keeps its quiet ambient audio
// via data-has-audio). comp chip (eBay screenshot + value) top-right per card, running box-cost
// tracker top-left, profit/ROI recap on black after the last card.
//
// SCALE (env, default 1): 1 -> author at 1080x1920. 2 -> author NATIVELY at 2160x3840 for true 4K
// (no --resolution DPR upscaling; source video is used 1:1, overlays drawn at native 4K). Every px
// dimension AND every GSAP px offset is multiplied by S so the layout is proportionally identical.
...
<html lang="en" data-resolution="portrait">
  <head>
    <meta charset="UTF-8" />
    <title>Final Edit (Shorts) - 2025 Topps Signature Class${S > 1 ? " (4K)" : ""}</title>
  ...
  <body>
    <div id="root" data-composition-id="final-Signature2025" data-width="${W}" data-height="${H}" data-start="0" data-duration="${DURATION}">
...
${saleDivs}
      ...
      <script>
        (function () {
          ...
          window.__timelines["final-Signature2025"]=tl;
        })();
      </script>
    </div>
  </body>
