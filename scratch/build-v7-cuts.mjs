...
const OVERRIDE_ZONES = [
  ...
  { start: 117.0, end: 136.0, reason: "Barnes/Baugh — dedupe back-flip + Baugh re-show" },
...
// Fan shots bumped from 0.5s → 0.8s per user feedback.
const MANUAL_SEGMENTS = [
  ...
  { start: 129.0, end: 132.0, note: "Barnes -> Baugh natural slide (2:09-2:12, no back-flip)" },
...
const out = {
  comment: `v7 — v6 + Barnes/Baugh dedupe (3s natural slide, no back-flip) + fan shots 0.5s -> 0.8s. ${cards.length} card windows (${MANUAL_SEGMENTS.length} manual, ${cards.length - MANUAL_SEGMENTS.length} auto). Total ${totalDur.toFixed(2)}s.`,
  segments,
};
