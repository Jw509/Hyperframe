...
const j=JSON.parse(readFileSync("scratch/autocut/cut-full-v5.json","utf8"));
...
writeFileSync("scratch/autocut/cut-full-v5-final.json",JSON.stringify({comment:j.comment,segments:s},null,2));
const land=s.filter(x=>x.source==="landscape");
console.log(`v5 final: ${s.length} beats, ${total.toFixed(1)}s (${(total/60).toFixed(2)} min); clamped ${fixed} same-region overlaps, dropped ${dropped} slivers`);
